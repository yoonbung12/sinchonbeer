...
class MybatisOrderDaoTest {
	...
	void 주문등록() {
		// given
		OrderInfo order = new OrderInfo("shop", 11000, 1, 3, 2, 1); // category, price, tourIdx, tourPeople, memberIdx, addressIdx 
	...
	void 주문조회() {
		// given
		OrderInfo order = new OrderInfo("shop", 24000, 1, 3, 2, 1); // category, price, tourIdx, tourPeople, memberIdx, addressIdx 
		dao.save(order);
	...
	void 멤버인덱스로_주문조회() {
		...
		OrderInfo order1 = new OrderInfo("shop", 22000, 1, 5, 3, 1); // category, price, tourIdx, tourPeople, memberIdx, addressIdx 
		OrderInfo order2 = new OrderInfo("shop", 22000, 1, 3, 3, 1); // category, price, tourIdx, tourPeople, memberIdx, addressIdx 
		dao.save(order1);
		dao.save(order2);
	...
	void 카테고리와_멤버인덱스로_주문조회() {
		...
		// 멤버 테이블에 의존하고 있기 때문에 테스트할 때마다 달라질 우려가 있으므로 향후에 테스트 방식을 바꿔야 함
		OrderInfo order1 = new OrderInfo("shop", 22000, 1, 5, 3, 1); // category, price, tourIdx, tourPeople, memberIdx, addressIdx 
		OrderInfo order2 = new OrderInfo("tour", 22000, 1, 3, 3, 1); // category, price, tourIdx, tourPeople, memberIdx, addressIdx 
		dao.save(order1);
		dao.save(order2);
